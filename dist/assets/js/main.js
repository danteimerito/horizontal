...
//Go to home page, then navigate to 'about' section
/* 
$(document).ready() 
function homeAbout() {
	document.getElementById('wyatt-toggle').style.backgroundColor = "red";
}
*/


//Simulate click on hamburger menu icon to close responsive nav after clicking links to page sections on mobile view only (#about, #contact, etc.)
var x = window.matchMedia("(max-width: 990px)")
function wyattToggle() {
	if (x.matches) { // If media query matches
		document.getElementById("wyatt-toggle").click();
	}
}
...
(function ($) {
	...
	// Link scroll.
	// SCROLL TO SECTION ANNIMATION /////////////////////////////////////////////////
	// Must query $body instead of $wrapper to trigger annimation from nav bar outside of $wrapper
	...
	$body
		...
		.on('click', 'a[href^="#"]', function (event) {
			...
			if (breakpoints.active('<=small')) {
				...
				x = $target.offset().top - (Math.max(0, $window.height() - $target.outerHeight()) / 2 + 45);
				y = { scrollTop: x };

			}
			else {
				x = $target.offset().left - (Math.max(0, $window.width() - $target.outerWidth()) / 2.25);
				y = { scrollLeft: x };
			}
